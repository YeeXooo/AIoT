...
public class LifeDetectionServiceImpl implements LifeDetectionService {
    ...
    @Override
    public Result<DetectionResult, AppError> evaluateLifeDetection(SensorReading radarSignal, DetectionWindow window, VehicleId vehicleId) {
        if (radarSignal == null) {
        ...
        Map<String, Double> values = radarSignal.values();
        if (!values.isEmpty()) {
            double energy = 0.0;
            for (double v : values.values()) {
                energy += v * v;
            }
            energy = Math.sqrt(energy / values.size());
