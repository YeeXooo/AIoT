...
public class PrivacyProtectionServiceImpl implements PrivacyProtectionService {
    ...
    public Result<Void, AppError> validateDataDesensitization(SensorReading reading) {
        ...
        if (reading.getFeatureVector() != null && !reading.getFeatureVector().isEmpty()) {
            for (double feature : reading.getFeatureVector()) {
    ...
    public Result<Integer, AppError> purgeExpiredRecords() {
        List<RoadRageVoiceRecord> allRecords = roadRageVoiceRecordRepository.findAll();
        LocalDateTime now = LocalDateTime.now();
        int purgedCount = 0;

        for (RoadRageVoiceRecord record : allRecords) {
