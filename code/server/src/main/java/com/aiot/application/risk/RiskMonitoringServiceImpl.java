...
public class RiskMonitoringServiceImpl implements IRiskMonitoringService {
    ...
    @Override
    public Result<Void, AppError> processSensorReading(String sessionHandle, SensorReading reading) {
        ...
        for (RiskDeterminedEvent event : determination.determinedEvents()) {
            ...
            alert.setDriverId(session.driverId);
            alert.setVehicleId(session.vehicleId);
            alert.setTripId(session.tripId != null ? session.tripId : "");
            alert.setAlertType(event.alertType().name());
            alert.setRiskLevel(event.riskLevel().name());
            alert.setOccurredAt(LocalDateTime.now());
            alert.setAlertMsg(event.anomalyDescription());
            alertEventRepository.save(alert);
        }

        for (RiskResolvedEvent event : determination.resolvedEvents()) {
            alertEventRepository.findByDriverId(session.driverId).stream()
                    .filter(a -> a.getAlertType().equals(event.resolvedAlertType().name()))
                    .findFirst()
                    .ifPresent(alert -> alertEventRepository.delete(alert.getAlertId()));
        }

        if ("RADAR".equalsIgnoreCase(reading.getChannelType())) {
            var lifeResult = lifeDetectionService.evaluateLifeDetection(reading, session.detectionWindow);
            if (lifeResult.isOk()) {
                ...
                if (detection.detectedEvent() != null) {
                    ...
                    alert.setVehicleId(session.vehicleId);
                    alert.setTripId(session.tripId != null ? session.tripId : "");
                    alert.setAlertType(AlertType.LIFE_DETECTION.name());
                    alert.setRiskLevel(RiskLevel.L3_CRITICAL.name());
                    alert.setOccurredAt(LocalDateTime.now());
                    alert.setAlertMsg("Life detection alert for driver " + session.driverId);
                    alertEventRepository.save(alert);
                }
            }
        }
    ...
    public Result<QueryAlertHistoryResponse, AppError> queryAlertHistory(
        ...
        List<AlertEvent> alerts = alertEventRepository.findByDriverId(driverId.id());

        List<AlertEvent> filtered = alerts.stream()
                .filter(a -> {
                    if (a.getOccurredAt() == null) return true;
                    boolean afterFrom = from == null || !a.getOccurredAt().isBefore(from);
                    boolean beforeTo = to == null || !a.getOccurredAt().isAfter(to);
                    return afterFrom && beforeTo;
                })
                .filter(a -> alertType == null || alertType.isBlank() || alertType.equals(a.getAlertType()))
                .filter(a -> riskLevel == null || riskLevel.isBlank() || riskLevel.equals(a.getRiskLevel()))
                .sorted(Comparator.comparing(AlertEvent::getOccurredAt, Comparator.nullsLast(Comparator.reverseOrder())))
                .toList();
        ...
        if (fromIndex < totalCount) {
            items = filtered.subList(fromIndex, toIndex).stream()
                    .map(a -> new AlertHistoryItem(
                            a.getAlertId(), a.getAlertType(), a.getRiskLevel(),
                            a.getTripId(), a.getDriverId(), a.getVehicleId(),
                            a.getOccurredAt(), a.getAlertMsg()))
                    .toList();
        } else {
            items = List.of();
        }
