...
public class RemoteGuardianshipServiceImpl implements IRemoteGuardianshipService {
    ...
    @Override
    public Result<VehicleWindowResponse, AppError> controlVehicleWindow(
            AccountId accountId, DriverId driverId, String windowPosition) {
        var accountOpt = systemAccountRepository.findById(accountId.id());
        if (accountOpt.isEmpty()) {
            return Result.err(AppError.notFound("SystemAccount", accountId.id()));
        }
        return Result.ok(new VehicleWindowResponse(driverId.id(), windowPosition, "CONTROLLED"));
    }

    @Override
    public Result<WindowStatusResponse, AppError> queryWindowStatus(
            AccountId accountId, DriverId driverId) {
        var accountOpt = systemAccountRepository.findById(accountId.id());
        if (accountOpt.isEmpty()) {
            return Result.err(AppError.notFound("SystemAccount", accountId.id()));
        }
        return Result.ok(new WindowStatusResponse(driverId.id(), "CLOSED", true));
    }

    @Override
    public Result<SparkRTCTokenResponse, AppError> issueSparkRTCToken(
        ...
        long expireTime = java.time.Instant.now().plus(java.time.Duration.ofHours(2)).toEpochMilli();
        return Result.ok(new SparkRTCTokenResponse(token, channelName, expireTime));
    }
}
