# 车载安全监测系统 API/接口层 OOD 设计方案（a_v13 / v13）
...
- `sessionToken`：会话级授权凭证，用于后续媒体会话管理操作（挂断、续期等需携带此 token 验证操作权限），非直接传入 SparkRTC SDK
- `sparkRTCRoomId`：SparkRTC 房间 ID
- `sparkRTCJoinToken`：实际传入 SparkRTC SDK `join` 方法的入房 Token，前端调用 `sparkRTCClient.join(sparkRTCRoomId, sparkRTCJoinToken)` 时使用
...
- **查询约束**：
  - `startTime` 到 `endTime` 的最大跨度为 30 天，超出返回 `400 Bad Request`（`InvalidTimeRange`）
  - `page` 默认 1；`size` 默认 100，最大 500
  - 当结果集超过 `size` 上限时，`totalCount` 返回实际总数，`trajectoryPoints` 截断至 `size` 条，前端通过分页参数逐页获取完整数据
...
| 下指令 | `{deviceId}/cmd/intervention/down` | 1 | 干预指令 | InterventionInstruction 集合（由 S1 风险判定驱动，云端经 IoTDA 关联下发干预指令至车机端执行） |
...
| 用户登录（JWT签发） | POST | `/api/v1/auth/login` | `LoginRequest` | `LoginResponse` |
| Token 刷新 | POST | `/api/v1/auth/refresh` | `RefreshTokenRequest` | `RefreshTokenResponse` |
| 二次身份验证 | POST | `/api/v1/auth/secondary-verify` | `SecondaryVerifyRequest` | `SecondaryVerifyResponse` |

> **认证端点说明**：以上三个认证端点（login / refresh / secondary-verify）的完整请求/响应模型定义见 §1.7 Auth 认证服务。家属 APP 在登录流程中依次调用 login → 获得 JWT access token → 后续 API 调用携带 `Authorization: Bearer <token>`；高敏操作前通过 secondary-verify 获取 `secondaryAuthToken`；access token 过期前通过 refresh 端点换发新 token。
>
...
// === WebSocket 下行消息模型（续） ===
interface TokenRenewedMessage {
  sparkRTCRoomId: string
  sparkRTCJoinToken: string
  expiresAt: string                       // ISO 8601
}

interface SubscribeStatusAckMessage {
  subscriptionId: string
  initialSnapshot: DriverStatusSnapshot
}

interface RescueTriggeredMessage {
  rescueRequestId: string
...
| 家属告警/状态推送 GPS 权限校验 | MQTT `family/{accountId}/alert/push` 和 `family/{accountId}/status/push` 推送下发前 | 推送阶段同样需校验家属当前监护权限有效性：权限有效时携带完整 GPS；权限已过期/已撤销时 `gpsLocation` 字段置 `null`，仅推送告警类型/等级/时间等非位置信息。此规则与 Pull 场景（家属查询驾驶员位置）的规则对称一致 |

### 5.7 数据生命周期与保留策略

系统处理高度敏感数据（BR-04 隐私边界约束），以下定义各关键数据类别的最小保留策略：

| 数据类别 | 最小保留周期 | 归档策略 | 到期清理规则 |
|------|:--:|------|------|
| **告警历史** | 180 天 | 180 天后归档至冷存储（OBS 归档层），保留 1 年 | 归档满 1 年后自动删除 |
| **行程记录及评分** | 365 天 | 行程结束 90 天后归档至温存储 | 归档满 365 天后自动删除；匿名化聚合统计（按车队/时间维度）永久保留 |
| **生理体征快照** | 90 天 | 不归档 | 到期物理删除，无恢复窗口 |
| **车辆遥测数据** | 90 天 | 不归档 | 到期物理删除；聚合统计（如日均车速、里程累计）永久保留 |
| **家属监护关系** | 关系解除后保留 30 天 | 不归档 | 30 天审计窗口后物理删除关系记录及关联的权限历史 |
| **救援记录** | 3 年 | 救援记录落库后不归档（主存储） | 3 年后匿名化保留（仅保留统计特征，移除驾驶员/车辆可识别信息） |
| **路怒语音存证** | 90 天 | OBS 加密桶存储，不归档 | 90 天后自动删除（密钥同步销毁），参见 §5.5 |
| **DMS 脱敏特征向量** | 180 天 | 180 天后归档至冷存储，保留 1 年 | 归档满 1 年后自动删除 |

> **设计依据**：以上保留周期应参照适用法律法规（如《个人信息保护法》）进行最终调整。当前数值为基于系统业务需求的最小建议值，项目实施阶段需由法务合规团队确认。驾驶员注销后数据清理策略参见 §5.6 对应校验点。
...
以下变更基于审查文件（a_v13_iteration_requirement.md）的 8 个 LOCATED 问题：

| 审查意见 | 处理方式 |
|---------|---------|
| 问题 1（中等·MQTT 干预指令 Topic 存在无对应 API 端点的用例描述）：`cmd/intervention/down` 的 Payload 说明引用"如车队管理员远程鸣笛等"，但 S2 仅提供只读端点，S4 亦无干预下发端点 | **修改** — 将 §2.1 `cmd/intervention/down` 行的 Payload 说明从"云端关联下发干预指令，如车队管理员远程鸣笛等"改为"由 S1 风险判定驱动，云端经 IoTDA 关联下发干预指令至车机端执行"，移除暗示车队管理员手动发起干预的用例描述，使 Payload 说明与系统实际能力一致 |
| 问题 2（中等·sessionToken 与 sparkRTCJoinToken 双字段语义重叠）：`RequestMediaSessionResponse` 中两字段均描述为接入 SparkRTC 的凭证，未区分用途 | **修改** — 更新 §1.3 `RequestMediaSessionResponse` 的字段描述：(a) `sessionToken` 明确为会话级授权凭证，用于后续媒体会话管理操作（挂断、续期），非直接传入 SparkRTC SDK；(b) `sparkRTCJoinToken` 明确为实际传入 SparkRTC SDK `join` 方法的入房 Token，并给出调用示例 `sparkRTCClient.join(sparkRTCRoomId, sparkRTCJoinToken)`。ArkTS 代码示例（§4.1）中已展示正确用法，本次仅澄清字段级语义 |
| 问题 3（中等·§4.1 家属 APP REST API 调用清单缺少认证端点）：`requirement.md:38` 要求覆盖全部后端接口，但当前清单缺少 `login`、`refresh`、`secondary-verify` 三个端点 | **修改** — 在 §4.1 REST API 调用列表中新增三行（用户登录/Token 刷新/二次身份验证），并新增"认证端点说明"块指向 §1.7 完整模型定义，阐明家属 APP 认证链路的调用流程 |
| 问题 4（一般·JWT scope 字段与角色权限模型的关系未定义）：JWT Payload 含 `scope` 数组，但角色→权限映射表未说明 scope 的角色分配规则 | **修改** — 移除 JWT Payload 示例中的 `scope` 字段。角色到端点范围的映射已由 §5.1 角色→权限映射表完整定义，`scope` 字段为冗余设计，移除可避免 API 使用者对其与角色的优先级产生歧义 |
| 问题 5（一般·缺乏数据生命周期/保留策略）：告警历史、行程评分、生理体征快照、GPS 轨迹、家属监护关系、救援记录等关键数据的保留周期均未定义 | **修改** — 在 §5.6 之后新增 §5.7"数据生命周期与保留策略"小节，覆盖告警历史、行程记录及评分、生理体征快照、车辆遥测数据、家属监护关系、救援记录、路怒语音存证、DMS 脱敏特征向量共八类数据的最小保留周期、归档策略及到期清理规则，并附法律法规合规提示 |
| 问题 6（一般·QueryTrajectoryResponse 缺少时间范围约束和分页限制）：未声明最大查询时间跨度、size 上限及截断策略 | **修改** — 在 §1.4 `QueryTrajectoryResponse` 段落后新增"查询约束"块：(a) `startTime` 到 `endTime` 最大跨度 30 天；(b) `size` 默认 100、最大 500；(c) 超过阈值时 `totalCount` 返回实际总数、`trajectoryPoints` 截断至 `size` 条，前端通过分页获取完整数据 |
| 问题 7（一般·ArkTS WebSocket 消息模型缺少 token_renewed 等多个类型定义）：§3.1 定义了下行消息类型但 §4.1 接口定义区遗漏 | **修改** — 在 §4.1 ArkTS 数据模型定义区新增 `TokenRenewedMessage`、`SubscribeStatusAckMessage`、`RescueTriggeredMessage` 三个 TypeScript 接口，字段定义与 §3.1 下行消息表中对应 Payload 完全一致 |
| 问题 8（轻微·隐私边界校验表缺少家属告警推送中的 GPS 权限校验规则）：校验表覆盖 Pull 场景但缺少 Push 场景的 GPS 脱敏规则 | **修改** — 在 §5.6 隐私边界安全校验点表中新增一行"家属告警/状态推送 GPS 权限校验"，明确推送阶段同样需校验家属当前监护权限有效性，权限无效时 `gpsLocation` 置 `null`，与 Pull 场景规则对称一致 |
