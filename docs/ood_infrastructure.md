...
| 感知数据模拟源 | Java | 本期以模拟数据注入感知输入，按固定频率生成 SensorReading。DMS 视觉通道可替换/并列为基于 YOLO 的真实视觉感知实现（Python sidecar + DmsPerceptionAdapter），二者产出相同的 SensorReading(DMS_CAMERA) 契约，详见分册 `docs/ood_perception_yolo.md` |
