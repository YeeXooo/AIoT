# 车载安全监测系统 应用层 OOD 设计方案（a_v5 / v1）

> 本文档为「智能物联——基于多传感器融合的车载安全监测系统」的**应用层**架构级 OOD 设计方案，承接领域层 OOD 产出（`a_v10_design_v1.md`），遵循 DDD 分层思想。应用层持有一组应用服务契约（interface）及对应实现（class），每个应用服务作为对应功能域的**入口门面**，负责编排领域服务、仓储和基础设施端口，处理事务边界、安全门控和外部请求适配，自身不包含领域业务逻辑。**后端实现语言为 Java，框架采用 Spring Boot**，应用服务以 `@Service` 标注、依赖注入采用 Spring 构造器注入。方案在 Java 类型系统能力范围内做抽象，不涉及具体实现代码。
...
- **契约先行**：每个应用服务以Java `interface` 定义对外契约（仅声明方法签名），由 `class` 实现具体编排逻辑。契约与实现分离支持测试 mock 和未来扩展。
...
| 接口契约定义 | 应用层 | 以Java `interface` 定义六个应用服务的对外契约 |
...
应用层采用 Spring **构造器注入**模式：每个应用服务 `@Service` class 的构造器接收其依赖的 interface（领域服务、仓储、端口），通过 `@Autowired`（构造器注入时可选）由 Spring IoC 容器自动装配。

具体模式：

- **构造器注入**：在应用服务 class 的构造器上声明依赖的 interface，Spring 自动解析并注入实现类实例。适合当前项目规模，类型安全且便于单元测试时手动 mock。
- **组件扫描**：应用服务实现类标注 `@Service`，领域服务实现类标注 `@Component`，仓储实现类标注 `@Repository`，Spring 自动扫描并注册到 IoC 容器。
...
| 类别 | 命名模式 | 示例 | 说明 |
|------|---------|------|------|
| 应用服务接口 | `I` + 服务名 | `IRiskMonitoringService` | Java `interface`，仅声明方法签名 |
| 应用服务实现 | 服务名 + `Impl` | `RiskMonitoringServiceImpl` | Java `class`，实现编排逻辑 |
...
| 错误类型 | `AppError` 枚举变体 | `AppError.SessionNotFound` | Java `enum`，携带上下文信息 |

### 3.1 S1 — IRiskMonitoringService / RiskMonitoringServiceImpl（风险监测服务）

#### 接口契约：IRiskMonitoringService

`IRiskMonitoringService` 是Java `interface`，声明风险监测功能域的对内契约。其实现类 `RiskMonitoringServiceImpl` 负责编排领域服务、仓储和端口。
...
- 对接边缘侧感知采集层，接收流式感知数据（覆盖疲劳、分心、异常驾驶行为、路怒四类风险）并分发至领域层的 RiskDeterminationService 进行判定
...
- **依赖的领域服务**：RiskDeterminationService（流式融合判定门面，协调 FatigueDeterminationService、DistractionDeterminationService、AbnormalDrivingDeterminationService、RageDeterminationService 等子判定服务）、LifeDetectionService（活体遗留判定）、EmergencyResponseService（碰撞失能判定）
- **依赖的仓储**：TripRepository（查询当前行程告警列表）、DriverRepository（查询驾驶员信息）
- **依赖的领域事件**：订阅 AlertTriggeredEvent（用于将告警推送至云端通知通道）、VehicleIgnitionOffLockedEvent（用于触发活体检测会话启动）、SensorFaultEvent（传感器自检故障信号，3 秒内编排 HMI 语音告警——"传感器故障，请谨慎驾驶"）
...
| `startLifeDetection` | `StartLifeDetectionRequest` | `Result<StartLifeDetectionResponse, AppError>` | — | **(仅供内部事件驱动调用，不供外部调用方直接调用)** 车辆熄火落锁后经由 VehicleIgnitionOffLockedEvent → EventBus 触发启动活体检测会话。委托 LifeDetectionService.startDetectionWindow 在 60s 窗口内执行判定，返回窗口句柄和窗口持续时间。外部接口层不直接调用此方法，活体检测的启动统一由事件驱动 |
...
| `queryAlertHistory` | `QueryAlertHistoryRequest` | `Result<QueryAlertHistoryResponse, AppError>` | 只读 | 按驾驶员标识 + 时间范围查询历史 SafetyAlertEvent 列表，支持按 AlertType / RiskLevel 过滤和分页。委托 AlertProjectionRepository 查询 CQRS 读模型投影 |
...
`IInterventionService` 是Java `interface`，声明干预执行功能域的对内契约。
...
- **路怒判定后干预编排**：当领域层产出 RageDeterminedEvent 时，编排多通道干预——AC_ADJUSTMENT（空调温度/风量调节以安抚情绪）、INFOTAINMENT_PLAYBACK（播放舒缓音乐/语音安抚播报），同时编排语音存证录制链路

#### 协作关系

- **依赖的领域服务**：DS-07 InterventionService（领域层干预执行服务）
- **依赖的仓储**：TripRepository（查询当前行程的干预指令历史）
- **依赖的领域事件**：订阅 RiskDeterminedEvent / RiskResolvedEvent（用于边缘侧同步触发干预指令生成）、LifeDetectedEvent（活体遗留检测成立时触发边缘侧紧急干预指令，如双闪+鸣笛）、RageDeterminedEvent（路怒判定成立时触发多通道干预——AC_ADJUSTMENT + INFOTAINMENT_PLAYBACK——并编排语音存证录制链路）、SensorFaultEvent（传感器自检故障信号，3 秒内编排 HMI 语音告警——"传感器故障，请谨慎驾驶"）
...
当领域层产出 RageDeterminedEvent（L2 及以上）时，S2 InterventionServiceImpl 编排语音存证录制链路：

| 生命周期阶段 | 处理策略 |
|-------------|---------|
| **触发条件** | S2 消费 RageDeterminedEvent（由 S1 处理 ACOUSTIC/MICROPHONE 传感器数据 → 领域层 RageDeterminationService 判定后产出），风险等级达到 L2 及以上时触发存证录制 |
| **录制范围** | 从 RageDeterminedEvent 时间戳起，录制车内音频 60s（窗口含触发前后各 30s），经由边缘侧 ACOUSTIC 传感器信道采集原始音频流 |
| **边缘存储** | 音频片段以加密形式暂存于边缘终端本地存储，每个片段独立加密（AES-256-GCM），密钥由云端密钥管理服务（KMS）下发 |
| **云端上传** | 在当前行程结束时或边缘存储空间不足时，由 S2 通过 IoTDA 通道将加密音频片段批量上传至云端对象存储（OBS），上传完成后从边缘侧安全擦除 |
| **保留策略** | 云端存证保留 90 天（与 BR-04 隐私边界对齐——过期自动清除），家属/车队管理员查看需经二次授权（DS-08 PermissionService 校验） |
| **隐私边界衔接** | 存证访问遵循 BR-04 原则：仅在①交通事故定责、②路怒投诉受理、③司法协查三类场景下经二次授权后可调取。存证元数据（触发时间、风险等级、行程 ID）与音频正文分离存储——元数据可被常规查询，音频正文仅在授权校验通过后解密读取 |

### 3.3 S3 — IRemoteGuardianshipService / RemoteGuardianshipServiceImpl（远程监护服务）

#### 接口契约：IRemoteGuardianshipService

`IRemoteGuardianshipService` 是Java `interface`，声明远程监护功能域的对内契约。
...
- **远程车窗控制**：家属在授权范围内远程操控车窗开闭，本服务执行安全门控和二次验证后委托领域层执行
...
| **离线消息补推** | 断开期间的告警事件会暂存于离线消息队列（每条告警保留 7 天），家属重连后按时间倒序补推（最多 20 条），超出部分仅推摘要通知。**隐私约束**：(1) 离线消息队列中的告警数据加密存储（AES-256-GCM），到期（7 天）自动清除；(2) 补推时按当前有效授权范围过滤——仅推送家属仍持有监护权限的驾驶员告警；(3) 已撤销监护权限的家属账户不补推其在撤销前产生但未投递的告警 |
...
| `controlVehicleWindow` | `ControlVehicleWindowRequest` | `Result<Unit, AppError>` | — | 家属远程操控车窗开闭。安全门控：角色校验 → 二次身份验证 → 权限判定（委托 DS-08.compareAgainstScope 校核 windowOperation 是否在授权范围内）。授权通过后委托领域层执行车窗控制，通过 IoTDA 下发 CAN 指令至车辆。**返回值语义**：`Ok(Unit)` 表示"指令已成功下发至 IoTDA"，不等同于"车窗操作完成"——前端应进一步通过 `queryWindowStatus` 查询车窗的实际执行状态与到位情况。车窗控制超时（30s 内未收到 IoTDA 执行确认 Ack）后标记操作状态为 `TIMEOUT`，详情见异常处理策略 |

| `queryWindowStatus` | `QueryWindowStatusRequest` | `Result<QueryWindowStatusResponse, AppError>` | 只读 | 家属查询指定车辆的车窗当前状态（各窗位置的 OPEN/CLOSED/PARTIAL/UNKNOWN 及最近操作状态）。委托领域层查询车辆聚合中的车窗状态，用于确认 `controlVehicleWindow` 异步指令的执行结果 |
| `queryGuardianshipPermissions` | `QueryGuardianshipPermissionsRequest` | `Result<QueryGuardianshipPermissionsResponse, AppError>` | 只读 | 家属查询其对指定驾驶员的监护权限。`driverId` 从路径参数获取，`accountId` 从 JWT `sub` 提取（非请求体传入）。校验：仅返回与请求方 `accountId` 关联的监护权限，拒绝查询非本人持有的权限关系（隐私边界见 §5.6） |
| `issueSparkRTCToken` | `IssueSparkRTCTokenRequest` | `Result<IssueSparkRTCTokenResponse, AppError>` | — | 为前端签发加入 SparkRTC 房间的临时 Token。后端集成华为云 SparkRTC SDK Token 生成 API，以服务端密钥签名。Token 有效期 10 分钟，与房间最大会话时长一致。`role` 取 `subscriber`（家属端）或 `publisher`（车机端） |
...
- 车窗控制指令下发后超时未收到执行确认（30s）→ 返回 `AppError.WindowControlTimeout(vehicleId, operation)`，前端展示"车窗操作超时，请刷新状态后重试"

### 3.4 S4 — IFleetManagementService / FleetManagementServiceImpl（车队管理服务）

#### 接口契约：IFleetManagementService

`IFleetManagementService` 是Java `interface`，声明车队管理功能域的对内契约。
...
- **依赖的领域事件**：订阅 PerformanceWarningEvent（由 DS-09 ScoringService 在每次评分计算完成后判断——low-score 即低于 60 分时即时触发并产出）、AlertTriggeredEvent（L3 告警用于缓存失效）、RiskResolvedEvent（L3 解除用于缓存失效）、SensorFaultEvent / VehicleMonitoringOfflineEvent（用于监测脱线车辆列表的实时更新——3 秒内反映在看板脱线车辆列表中）
...
| `getOfflineVehicles` | `GetOfflineVehiclesRequest` | `Result<GetOfflineVehiclesResponse, AppError>` | 只读 | 返回车队内当前传感器故障或监控脱线的车辆列表。数据来源为 SensorFaultEvent / VehicleMonitoringOfflineEvent 驱动的实时状态投影。脱线判定 SLA：传感器自检故障信号到达后 3 秒内反映在列表中 |
| `queryVehicleTrajectory` | `QueryTrajectoryRequest` | `Result<QueryTrajectoryResponse, AppError>` | 只读 | 按车辆标识或驾驶员标识 + 时间范围查询历史轨迹点序列。轨迹点包含 GPS 坐标、时间戳、车速。委托 TrajectoryQueryService 查询 CQRS 读模型投影，支持分页 |
...
`IEmergencyRescueService` 是Java `interface`，声明应急救援功能域的对内契约。
...
- **依赖的领域事件**：订阅 RescueReportReadyEvent（DS-12 消费 EmergencyActivatedEvent 后产出，驱动 SOS 救援报告投递至 120）、FamilyManualRescueRequestedEvent（家属手动救援触发）、LifeDetectedEvent（活体遗留检测成立时驱动救援预判——若家属在告警推送后规定时间内无响应，则自动升报救援）
...
| `createRescueReport` | `CreateRescueReportRequest` | `Result<CreateRescueReportResponse, AppError>` | 写 | 供 S3 RemoteGuardianshipService 在处理手动救援触发时内部编排调用，创建救援报告并获取 `rescueReportId`。委托 DS-12.triggerManualRescue 创建 RescueReport 聚合。不对外暴露为独立 REST 端点。**事务边界**：单 RescueReport 聚合写，读已提交 |
...
`IOTAManagementService` 是Java `interface`，声明 OTA 升级管理功能域的对内契约。
...
- **依赖的领域事件**：订阅 OTAUpgradeStartedEvent、OTAUpgradeCompletedEvent、OTAUpgradeFailedEvent、OTAUpgradeRolledBackEvent。其中 OTAUpgradeStartedEvent 用于在本服务记录升级已开始并初始化进度追踪；S2 InterventionServiceImpl 则订阅 OTAUpgradeCompletedEvent 和 OTAUpgradeRolledBackEvent 以管理 CAN 干预抑制状态（分工详见 §7.2 链路 D）
...
| `cancelUpgradeTask` | `CancelUpgradeTaskRequest` | `Result<CancelUpgradeTaskResponse, AppError>` | 写 | 取消未完成的升级任务。校验：仅 `PENDING` 或 `TRANSMITTING` 阶段的任务允许取消；已进入 `VERIFYING`、`UPGRADING` 及终态（`COMPLETED` / `ROLLED_BACK`）的任务拒绝取消，返回 `AppError.UpgradeTaskNotCancellable`。**事务边界**：单 UpgradeTask 聚合写，读已提交 |
...
- 任务不可取消（非 PENDING / TRANSMITTING 阶段）→ 返回 `AppError.UpgradeTaskNotCancellable(taskId, currentStage)`

---

## 四、DTO 定义

以下 DTO 按应用服务分节列出。所有 DTO 均为Java不可变数据类（`class`，全字段构造器，无 setter），用于接口层与应用层之间的数据传输。
...
- `activeAlerts: List<(AlertType, RiskLevel)>` — 当前活跃风险及等级映射
- `derivedStatusColor: StatusColor` — 派生状态色（GREEN / YELLOW / ORANGE / RED）

**QueryAlertHistoryRequest**
- `driverId: DriverId`
- `timeRange: TimeRange` — 起始和结束时间戳
- `alertTypeFilter: Optional<AlertType>` — 可选告警类型过滤
- `riskLevelFilter: Optional<RiskLevel>` — 可选风险等级过滤
- `page: PageRequest` — 分页参数（页码 + 每页条数）

**QueryAlertHistoryResponse**
- `alerts: List<AlertSummary>` — 告警摘要列表
...
- `windowDurationSeconds: int` — 窗口持续时间（60）
...
- `activeInterventions: List<InterventionSummary>` — 当前活跃的干预指令摘要
...
- `familyAccountId: AccountId`
- `driverId: DriverId`
- `preferredRiskLevels: List<RiskLevel>` — 希望接收告警的风险等级集合
...
- `rescueRequestId: RescueRequestId` — S3 手动救援触发请求 ID
- `rescueReportId: RescueReportId` — 对应的 S5 救援报告 ID（S3 内部调用 S5.createRescueReport 后获取）
...
- `familyAccountId: AccountId` — 家属账户标识
- `driverId: DriverId` — 驾驶员标识
- `windowOperation: WindowControlOperation` — 车窗操作类型（OPEN / CLOSE / PARTIAL_OPEN）
- `secondaryAuthToken: String` — 二次身份验证凭证

**WindowControlOperation**（enum）：`OPEN` | `CLOSE` | `PARTIAL_OPEN`

**QueryWindowStatusRequest**
- `vehicleId: VehicleId` — 车辆标识

**QueryWindowStatusResponse**
- `windowStatuses: List<WindowStatusEntry>` — 各车窗状态列表

**WindowStatusEntry**
- `windowPosition: WindowPosition` — 车窗位置枚举
- `state: WindowState` — 车窗状态
- `lastOperation: Optional<WindowControlOperation>` — 最近一次操作
- `lastOperationResult: Optional<WindowOperationResult>` — 最近操作结果
- `updatedAt: Timestamp` — 状态更新时间

**WindowPosition**（enum）：`FRONT_LEFT` | `FRONT_RIGHT` | `REAR_LEFT` | `REAR_RIGHT`

**WindowState**（enum）：`OPEN` | `CLOSED` | `PARTIAL` | `UNKNOWN` | `MOVING`

**WindowOperationResult**（enum）：`SUCCESS` | `TIMEOUT` | `FAILED` | `PENDING`

**QueryGuardianshipPermissionsRequest**
- `driverId: DriverId` — 驾驶员标识（从路径参数获取）
- `accountId: AccountId` — 请求方账户标识（从 JWT `sub` 提取，非请求体传入）

**QueryGuardianshipPermissionsResponse**
- `familyAccountId: AccountId` — 家属账户标识
- `driverId: DriverId` — 驾驶员标识
- `permissions: List<GuardianshipPermissionEntry>` — 家属对该驾驶员的监护权限列表
- `careRelationship: CareRelationshipSummary` — 家属与驾驶员的监护关系概要

**GuardianshipPermissionEntry**
- `permissionType: GuardianshipPermissionType` — 权限类型：MEDIA_CALL / WINDOW_CONTROL / MANUAL_RESCUE / STATUS_MONITORING
- `granted: boolean` — 是否已授予
- `grantedAt: Timestamp` — 授予时间
- `expiresAt: Optional<Timestamp>` — 过期时间（永久权限时为 None）

**GuardianshipPermissionType**（enum）：`MEDIA_CALL` | `WINDOW_CONTROL` | `MANUAL_RESCUE` | `STATUS_MONITORING`

**CareRelationshipSummary**
- `status: CareRelationshipStatus` — 关系状态：ACTIVE / SUSPENDED / REVOKED
- `establishedAt: Timestamp` — 关系建立时间

**CareRelationshipStatus**（enum）：`ACTIVE` | `SUSPENDED` | `REVOKED`

**IssueSparkRTCTokenRequest**
- `roomId: String` — SparkRTC 房间 ID
- `userId: String` — 用户 ID
- `role: SparkRTCRole` — 角色：subscriber（家属端） / publisher（车机端）

**SparkRTCRole**（enum）：`subscriber` | `publisher`

**IssueSparkRTCTokenResponse**
- `token: String` — SparkRTC 临时 Token
- `expiresAt: Timestamp` — Token 过期时间

> **隐私保护**：`queryGuardianshipPermissions` 仅返回与请求方 `accountId` 关联的监护权限；拒绝查询非本人持有的权限关系（见 §5.6 隐私校验规则）。
...
- `distribution: Map<RiskLevel, double>` — 各风险等级占比（如 L1: 0.45 表示 45%）
- `heatmapData: List<HeatmapPoint>` — 风险热力图坐标序列
- `dataFreshness: DataFreshness` — FRESH / STALE（见 §3.4 异常处理策略）

**DataFreshness**（enum）：`FRESH` | `STALE`

**GetOfflineVehiclesRequest**
- `fleetId: FleetId`

**GetOfflineVehiclesResponse**
- `offlineVehicles: List<OfflineVehicleInfo>` — 脱线车辆列表

**OfflineVehicleInfo**（结构类型）
- `vehicleId: VehicleId` — 车辆标识
- `licensePlate: String` — 车牌号（人类可读标识）
- `driverId: DriverId` — 驾驶员标识
- `driverName: String` — 驾驶员姓名（人类可读标识）
- `offlineReason: OfflineReason` — 脱线原因
- `offlineSince: Timestamp` — 脱线起始时间
- `lastHeartbeat: Timestamp` — 最后心跳时间

**OfflineReason**（enum）：`SENSOR_FAULT` | `COMMUNICATION_LOST`

**HeatmapPoint**（结构类型）
- `latitude: double` — 纬度
- `longitude: double` — 经度
- `riskIntensity: double` — 风险强度（0.0~1.0）
...
- `drivers: List<HighRiskDriverSummary>`
- `totalCount: UInt64`

**HighRiskDriverSummary**
- `driverId: DriverId`
- `driverName: String` — 驾驶员姓名（人类可读标识）
- `compositeRiskScore: double` — 综合风险评分
- `latestTripSummary: TripDigest` — 最近行程摘要
- `primaryPenaltyItems: List<String>` — 主要扣分项描述
...
- `downloadUrl: Optional<String>` — PDF/Excel 下载链接（null if 报告为空）
- `isEmpty: boolean` — 所选时间范围内是否有行驶记录
...
- `drivingBehaviorSummary: DrivingBehaviorSummary` — 驾驶行为评分汇总
  - `overallScore: double` — 综合评分（0~100）
  - `subScores: Map<String, double>` — 各维度子评分（如 "fatigueScore", "distractionScore", "abnormalDrivingScore"）
  - `trendVsLastPeriod: double` — 对比上一周期的评分变化（正值为提升、负值为退步）
- `riskDistribution: Map<AlertType, UInt64>` — 各类风险告警的发生频次分布
- `penaltyBreakdown: List<PenaltyItem>` — 各维度扣分明细
  - `PenaltyItem` 含：`category: String`（如 疲劳/分心/异常驾驶）、`penaltyScore: double`（扣减分数）、`topViolations: List<String>`（主要违规描述列表）
- `totalMileage: Optional<double>` — 周期内总行驶里程（km）
- `totalDrivingTime: Optional<Duration>` — 周期内总驾驶时长
...
- `vehicleId: Optional<VehicleId>` — 车辆标识（与 driverId 至少提供一个）
- `driverId: Optional<DriverId>` — 驾驶员标识（与 vehicleId 至少提供一个）
- `timeRange: TimeRange` — 查询时间范围
- `page: PageRequest` — 分页参数

> **参数交叉校验**：若 `vehicleId` 和 `driverId` 同时提供但不匹配（vehicleId 对应的车辆当前/历史行程中不包含 driverId 对应的驾驶员），返回空轨迹序列并标注 `dataConsistency = INCONSISTENT`，而非返回 `AppError`。原因：两个参数均由调用方提供，不一致时优先返回空结果而非中断查询流程。

**QueryTrajectoryResponse**
- `trajectoryPoints: List<TrajectoryPoint>` — 轨迹点序列（时间正序）
- `totalCount: UInt64`
...
- `rescueReportId: RescueReportId`
- `authorizedOperations: List<RescueOperation>` — 授权操作集合：RemoteUnlock / HealthProfileAccess
- `validityDurationSeconds: int` — 有效期秒数（默认 1800 = 30min）

**RescueOperation**（enum）：`RemoteUnlock` | `HealthProfileAccess` | `RemoteWindowControl`

> **注意**：`RemoteWindowControl` 救援操作与 S3 中家属日常车窗控制（`controlVehicleWindow`）共享车窗控制通道但权限校验路径不同——救援场景下由 RescueAuthorizationToken 授权，日常场景下由 PermissionService 判定的家庭监护授权范围控制。
...
- `driverId: Optional<DriverId>`
- `vehicleId: Optional<VehicleId>`
- `timeRange: Optional<TimeRange>`
- `page: PageRequest`

**QueryRescueHistoryResponse**
- `rescueRecords: List<RescueRecordSummary>`
- `totalCount: UInt64`
...
**CreateRescueReportRequest**
- `driverId: DriverId` — 驾驶员标识（由 S3 从 `TriggerManualRescueRequest` 传入）
- `requesterAccountId: AccountId` — 请求家属的账户标识（由 S3 从 JWT `sub` 提取后传入）
- `triggerType: RescueTriggerType` — 触发类型（手动救援场景固定为 `MANUAL`）
- `secondaryAuthToken: String` — 二次身份验证 Token（由 S3 校验后传入，用于审计追溯）

**CreateRescueReportResponse**
- `rescueReportId: RescueReportId` — 创建的救援报告标识
- `status: RescueRecordStatus` — 创建后初始状态（`SENT`）
- `occurredAt: Timestamp` — 救援报告创建时间

### 4.6 S6 OTAManagementService DTO

**CreateUpgradeTaskRequest**
- `targetVehicleIds: List<VehicleId>` — 目标车辆列表（上限 100）
- `targetVersion: OTAVersion`
- `upgradeOptions: Optional<UpgradeOptions>` — 可选升级参数（分批策略、时间窗口等）
- `idempotencyKey: String` — 幂等性标识，同一 key 的重复请求幂等返回已创建任务

**UpgradeOptions**（结构类型）
- `batchStrategy: Optional<BatchStrategy>` — 分批策略（如按车型、按地域分批）
- `scheduledWindow: Optional<TimeRange>` — 预约升级时间窗口
- `forceUpgrade: boolean` — 是否强制升级（默认 false）

**BatchStrategy**（enum）：`BY_MODEL` | `BY_REGION` | `BY_BATCH_SIZE`

**CreateUpgradeTaskResponse**
- `createdTaskIds: List<UpgradeTaskId>` — 成功创建的任务列表
- `skippedVehicles: List<(VehicleId, AppError)>` — 跳过的车辆及原因

**QueryUpgradeProgressRequest**
- `vehicleIds: List<VehicleId>`

**QueryUpgradeProgressResponse**
- `progressEntries: List<UpgradeProgressEntry>`

**UpgradeProgressEntry**
- `vehicleId: VehicleId`
- `currentStage: UpgradeStage` — 当前升级阶段
- `progressPercent: double` — 进度百分比（0~100）
- `estimatedRemainingSeconds: Optional<int>` — 预估剩余时间
...
- `entries: List<UpgradeHistoryEntry>`
- `totalCount: UInt64`
...
**CancelUpgradeTaskRequest**
- `taskId: UpgradeTaskId` — 升级任务标识（从路径参数获取）

**CancelUpgradeTaskResponse**
- `taskId: UpgradeTaskId` — 被取消的任务标识
- `previousStatus: UpgradeStage` — 取消前的任务阶段
- `cancelledAt: Timestamp` — 取消时间

> **取消约束**：仅 `PENDING` 或 `TRANSMITTING` 阶段的任务允许取消；已进入 `VERIFYING`、`UPGRADING` 及终态（`COMPLETED` / `ROLLED_BACK`）的任务拒绝取消，返回 `AppError.UpgradeTaskNotCancellable`。
...
| `sensorType` | `SensorType` | 传感器类型枚举：DMS_CAMERA / MILLIMETER_WAVE_RADAR / ACCELEROMETER / PHYSIOLOGICAL_MONITOR / MICROPHONE / ACOUSTIC / REAR_IR_CAMERA |
| `values` | `Map<String, double>` | 传感器读数键值对（如 "PERCLOS": 0.85, "yawnFreq": 3.2） |
...
| `gpsLocation` | `Optional<GeoPoint>` | GPS 经纬度坐标（无活跃行程时为 None） |
| `speed` | `Optional<double>` | 当前车速 km/h（无活跃行程时为 None） |
| `tripStatus` | `TripStatus` | 行程状态：NOT_STARTED / ACTIVE / COMPLETED |
| `physiologicalSummary` | `Optional<PhysiologicalDigest>` | 生理状态摘要（心率区间、体温等，仅当生理监测可用时非空） |
| `windowStatus` | `Optional<List<WindowStatusEntry>>` | 车窗状态列表（定义见 §4.3），用于家属查询车窗实时位置与控制操作结果 |
...
应用层定义统一的错误枚举 `AppError`（Java `enum`），用于所有六个应用服务方法的 `Result<T, AppError>` 返回类型。AppError 继承领域层的三类错误分类（A：输入无效、B：业务拒绝、C：系统故障），每个变体携带必要的上下文信息。

```java
public sealed interface AppError
        permits AppError.InvalidSessionHandle,
                AppError.InvalidTimeRange,
                AppError.VehicleNotFound,
                AppError.InvalidParameterCombination,
                AppError.SessionNotFound,
                AppError.PermissionDenied,
                AppError.SecondaryAuthRequired,
                AppError.SessionEstablishFailed,
                AppError.UpgradeInProgress,
                AppError.UpgradeAlreadyFinished,
                AppError.IncompatibleTarget,
                AppError.AccessDenied,
                AppError.ReportGenerationTimeout,
                AppError.SubscriptionLimitExceeded,
                AppError.BatchSizeExceeded,
                AppError.DataSourceUnavailable,
                AppError.MessageQueueUnreachable,
                AppError.IoTDAChannelFailure,
                AppError.RescueReportDeliveryFailed,
                AppError.WindowControlTimeout {

    // === A 类：输入无效 ===
    record InvalidSessionHandle(String sessionHandle) implements AppError {}
    record InvalidTimeRange(String reason) implements AppError {}
    record VehicleNotFound(VehicleId vehicleId) implements AppError {}
    record InvalidParameterCombination(String detail) implements AppError {}

    // === B 类：业务拒绝 ===
    record SessionNotFound(String sessionHandle) implements AppError {}
    record PermissionDenied(PermissionDenialReason reason) implements AppError {}
    record SecondaryAuthRequired() implements AppError {}
    record SessionEstablishFailed(String detail) implements AppError {}
    record UpgradeInProgress(VehicleId vehicleId) implements AppError {}
    record UpgradeAlreadyFinished(VehicleId vehicleId, OTAUpgradeStatus currentStatus) implements AppError {}
    record IncompatibleTarget(VehicleId vehicleId, OTAVersion targetVersion) implements AppError {}
    record AccessDenied(AccessDenialReason reason) implements AppError {}
    record ReportGenerationTimeout(String suggestion) implements AppError {}
    record SubscriptionLimitExceeded(DriverId driverId, int maxLimit) implements AppError {}
    record BatchSizeExceeded(int maxLimit) implements AppError {}

    // === C 类：系统故障 ===
    record DataSourceUnavailable(String sourceName) implements AppError {}
    record MessageQueueUnreachable() implements AppError {}
    record IoTDAChannelFailure() implements AppError {}
    record RescueReportDeliveryFailed(int attemptNumber) implements AppError {}
    record WindowControlTimeout(VehicleId vehicleId, WindowControlOperation operation) implements AppError {}
}

public enum PermissionDenialReason {
    NOT_RELATED,           // 无监护关系
    NO_AUTHORIZATION,      // 无对应授权
...
| S3 | 远程车窗控制授权范围不匹配 | `PermissionDenied(reason)` | 前端提示"操作超出授权范围" |
| S3 | 车窗控制指令超时未确认（>30s） | `WindowControlTimeout(vehicleId, operation)` | 前端提示"车窗操作超时，请刷新状态后重试" |
| S4 | 看板查询超时 | 返回缓存数据（`DataFreshness=STALE`），非 Error | 前端标注 "数据可能不是最新" |
| S4 | 轨迹查询无数据 | 返回空轨迹序列，非 Error | 前端展示 "该时段内无轨迹数据" |
| S4 | 脱线车辆查询 | 返回空列表，非 Error | 前端展示"当前无脱线车辆" |
...
> 图中事件由领域层产出后，经领域事件总线（EventBus）路由至订阅者应用服务，箭头表示事件路由方向而非应用服务间的直接方法调用。
...
[领域事件总线 — outbox + 消息队列异步投递]
    ...
    └──► S5 EmergencyRescueServiceImpl（通过 RescueReportReadyEvent 路径，该事件由 DS-12 消费 EmergencyActivatedEvent 后产出）→ RescueReportPort 投递 120
```
...
#### 链路 D：OTA 升级期间干预抑制与事件消费分工

S6 与 S2 对 OTA 领域事件的订阅分工如下：

- **S6 OTAManagementServiceImpl**：订阅全部四个核心 OTA 状态事件（OTAUpgradeStartedEvent / OTAUpgradeCompletedEvent / OTAUpgradeFailedEvent / OTAUpgradeRolledBackEvent），用于升级进度追踪、结果通知推送和历史记录更新
- **S2 InterventionServiceImpl**：仅订阅 CAN 干预恢复相关事件（OTAUpgradeCompletedEvent / OTAUpgradeRolledBackEvent），用于在升级完成或回滚完成时立即恢复 CAN 级干预能力

```
[领域层] OTAUpgradeStartedEvent / OTAUpgradeCompletedEvent / OTAUpgradeFailedEvent / OTAUpgradeRolledBackEvent (DS-15 产出)
    │
    ▼
[领域事件总线 — outbox 异步投递]
    │
    ├──► S6 OTAManagementServiceImpl (订阅全部四个事件 → 进度追踪 + 通知推送 + 历史记录)
    │
    └──► S2 InterventionServiceImpl (订阅 OTAUpgradeCompletedEvent / OTAUpgradeRolledBackEvent)
...
#### 链路 E：路怒判定 → 多通道干预 + 语音存证（边缘侧同步，≤500ms）

```
S1 RiskMonitoringServiceImpl
    │  接收 ACOUSTIC/MICROPHONE 传感器音频数据帧
    ▼
[领域层] RiskDeterminationService → RageDeterminationService 判定 → RageDeterminedEvent
    │  (边缘侧进程内同步消费)
    ▼
[领域层] InterventionService (DS-07) → 生成路怒干预指令集合
    │  AC_ADJUSTMENT (空调温控安抚) / INFOTAINMENT_PLAYBACK (舒缓音乐/语音播报)
    │
    ▼
[领域事件总线 — 边缘侧进程内同步]
    │
    ├──► S2 InterventionServiceImpl → 更新干预状态（供 HMI 查询）
    │        │
    │        └──► 编排语音存证录制：启动 60s ACOUSTIC 音频采集 → 边缘加密暂存
    │
    └──► 边缘侧 HMI 查询 InterventionServiceImpl.queryInterventionStatus
```
...
sequenceDiagram
    ...
    Buffer-->>DS06: List<PhysiologicalSnapshot>
...
1. **测试 mock 需求**：验收测试和单元测试需要对应用服务进行 mock，`interface` 是Java中实现 mock 的标准方式。若仅有 `class`，测试代码需依赖具体实现，无法独立验证接口层调用逻辑。
2. **契约清晰性**：`interface` 仅声明方法签名，天然形成与接口层之间的"最小依赖面"，接口层开发人员不需要了解实现细节。
3. **Java语言习惯**：Java的 `interface` 支持方法签名声明和 default 实现，与面向接口编程的理念契合。接口与实现分离是Java生态中的推荐模式。
4. **保守扩展**：当前确实不存在多态替换场景，但 `interface` 的引入成本极低（一份声明对应一份实现），为未来可能的替换需求预留空间，无需透支设计复杂度。

**Java 实现考量**：Java 的 `interface` 可声明方法签名（不含方法体），由 `class` 通过 `implements InterfaceName` 语法实现。依赖注入时，构造器参数声明为 `interface` 类型，Spring 容器自动注入 `@Component`/`@Service` 标注的实现类实例，实现编译期依赖抽象、运行时绑定具体实现。若未来为测试编写 mock 实现，只需新增一个实现同一 `interface` 的 `class` 而不需修改调用方代码。
...
| 审查意见 | 严重度 | 修改措施 |
|---------|:--:|---------|
| **接口方法未提供具体签名**：六个应用服务的接口契约全部以自然语言描述，未提供方法名、参数类型名或返回值类型名 | 高 | 在 §3.1–§3.6 中为每个应用服务的每个方法提供具体契约表，包含方法名、输入 DTO 类型名、返回类型（`Result<T, AppError>` 模式）、事务标注。方法签名以Java风格表述（如 `startMonitoringSession(req): Result<StartMonitoringResponse, AppError>`） |
...
| **异常类型仅有名称无定义**：未定义任何错误枚举或错误码体系 | 高 | 在 §6.1 中定义 `AppError` 枚举（Java `enum`），包含 A/B/C 三类共 15 个变体，每个变体携带结构化上下文信息（如 `PermissionDenied(reason: PermissionDenialReason)`）。附属定义 `PermissionDenialReason` 和 `AccessDenialReason` 两个子枚举 |
...
| **Java interface 能力未利用**：以 class 定义应用服务，未利用 interface 先行定义契约 | 低 | 修订决策 A1：六个应用服务均拆分为 `interface`（如 `IRiskMonitoringService`，声明方法签名）+ `class`（如 `RiskMonitoringServiceImpl`，实现编排逻辑）。在 §2.2 补充Java依赖注入方式（构造器注入 + 手动装配，可选 IoC 容器）。模块划分表（§2.1）同步更新为 interface + class 双列 |
...
本版本（a_v5 / v1）基于审查报告 b_v4_diag_v1（共 8 个问题）进行定向修订。

| 编号 | 审查意见 | 严重度 | 修改措施 |
|:--:|---------|:--:|---------|
| 1 | **BR-03 路怒判定与调节链路覆盖率不足**：SensorType 缺少 MICROPHONE/ACOUSTIC；S2 干预类型缺少 AC_ADJUSTMENT/INFOTAINMENT_PLAYBACK；路怒语音存证链路完全缺失 | 严重 | (1) 在 §5.1 SensorType 中新增 MICROPHONE、ACOUSTIC、REAR_IR_CAMERA 变体；(2) 在 §3.1 S1 边缘侧职责中将风险覆盖从三类扩展为四类（含路怒），协作关系中新增 RageDeterminationService；(3) 在 §3.2 S2 新增路怒判定后干预编排职责（AC_ADJUSTMENT + INFOTAINMENT_PLAYBACK）和「路怒语音存证生命周期」子节，覆盖触发条件、录制范围、边缘加密存储、云端上传、90 天保留策略、与 BR-04 隐私边界衔接；(4) 在 §3.2 S2 协作关系事件订阅列表中新增 RageDeterminedEvent；(5) 在 §7.2 新增「链路 E」描述路怒判定→多通道干预+语音存证的完整编排路径 |
| 2 | **§7.2 链路 B 描述与 §3.5 订阅声明及 §8.3 时序图存在逻辑矛盾**：§7.2 称 S5 通过 EmergencyActivatedEvent 路径触发，但 §3.5 订阅 RescueReportReadyEvent，§8.3 亦然 | 严重 | 将 §7.2 链路 B 中 S5 分支描述统一为「S5 EmergencyRescueServiceImpl（通过 RescueReportReadyEvent 路径，该事件由 DS-12 消费 EmergencyActivatedEvent 后产出）→ RescueReportPort 投递 120」；同时将 §3.5 S5 协作关系事件订阅列表中补充 LifeDetectedEvent（修复与 §8.2 时序图中救援预判路径的不一致） |
| 3 | **远程车窗控制功能未在应用服务接口中体现**：S3 六方法无车窗控制、S5 RescueOperation 不含车窗操作、§6.3 安全门控列举车窗控制却无对应方法 | 一般 | (1) 在 §3.3 S3 职责描述中新增「远程车窗控制」项；(2) 在 S3 方法契约表中新增 `controlVehicleWindow` 方法（含完整安全门控链：角色校验→二次验证→权限判定）；(3) 在 §4.3 新增 `ControlVehicleWindowRequest` DTO 和 `WindowControlOperation` 枚举（OPEN/CLOSE/PARTIAL_OPEN）；(4) 在 §4.5 中将 RescueOperation 枚举补充 `RemoteWindowControl` 变体；(5) 在 §6.2 错误策略表新增远程车窗控制授权范围不匹配处理 |
| 4 | **SensorType 枚举缺少后排红外摄像头类型**：需求 §3.1 明确新增「后排红外摄像头」独立数据源 | 一般 | 在 §5.1 SensorType 中新增 REAR_IR_CAMERA 变体（与第 1 项合并修改） |
| 5 | **BR-08 失效保护（fail-safe）的应用层编排链路缺失**：边缘侧 S1/S2 未定义接收传感器自检故障信号及 3 秒 HMI 语音报警；云端 S4 无监测脱线车辆查询方法与传感器故障事件订阅 | 一般 | (1) 在 §3.1 S1 和 §3.2 S2 协作关系事件订阅列表中补充 SensorFaultEvent，标注「3 秒内编排 HMI 语音告警——传感器故障，请谨慎驾驶」SLA；(2) 在 §3.4 S4 协作关系事件订阅列表中补充 SensorFaultEvent / VehicleMonitoringOfflineEvent，标注 3 秒内反映在看板脱线车辆列表中；(3) 在 §3.4 S4 方法契约表中新增 `getOfflineVehicles` 查询方法；(4) 在 §4.4 新增 `GetOfflineVehiclesRequest`、`GetOfflineVehiclesResponse`、`OfflineVehicleInfo`、`OfflineReason` 等 DTO 和枚举；(5) 在 §6.2 和 §7.4 中新增对应错误处理和事务条目 |
| 6 | **S1 协作关系中缺失 VehicleIgnitionOffLockedEvent 订阅声明**：§8.2 时序图显示 S1 通过该事件触发活体检测，但 §3.1 S1 事件订阅列表未列出 | 一般 | 在 §3.1 S1 协作关系事件订阅列表中补充 VehicleIgnitionOffLockedEvent |
| 7 | **60 分阈值绩效预警的事件生产链未在应用层明确**：S4 订阅 PerformanceWarningEvent 仅覆盖消费侧，事件由哪个领域服务在何种条件下产出未说明 | 一般 | 在 §3.4 S4 协作关系中补充说明 PerformanceWarningEvent 由 DS-09 ScoringService 在每次评分计算完成后判断（low-score→即时事件触发）并产出 |
| 8 | **§7.1 协作关系图未包含 EventBus 节点**：箭头从 S1 直接指向 S2/S3/S4，视觉效果与「应用服务间零直接调用」原则矛盾 | 轻微 | 重绘 §7.1 协作关系图，在 S1 与 S2/S3/S4 之间新增「领域事件总线 EventBus」节点作为事件路由中介，箭头从 S1 指向 EventBus 再分发至各订阅服务，使图结构层面消除直接箭头误导 |

---

## 十六、修订说明（a_v6 / v1）

本版本（a_v6 / v1）基于审查报告 b_v5_diag_v2 + b_v5_challenge_v2（共 8 个问题）进行定向修订。

| 编号 | 审查意见 | 严重度 | 修改措施 |
|:--:|---------|:--:|---------|
| 1 | **`startLifeDetection` 公开方法调用路径与事件驱动触发路径未明确界定**：§3.1 列为公开方法契约，§8.2 时序图通过 VehicleIgnitionOffLockedEvent → EventBus 触发，两条路径并存造成实现歧义 | 一般 | 在 §3.1 方法契约表中将 `startLifeDetection` 标注为「仅供内部事件驱动调用，不供外部调用方直接调用」，明确该方法的启动统一由 VehicleIgnitionOffLockedEvent 经 EventBus 触发，外部接口层不可直接调用。§8.2 时序图维持原事件驱动触发路径不变 |
| 2 | **`BatchStrategy` 类型完全未定义**：§4.6 UpgradeOptions 引用 `batchStrategy: Optional<BatchStrategy>`，但全文未给出该类型定义 | 一般 | 在 §4.6 UpgradeOptions 之后新增 `BatchStrategy` 枚举类型定义：`BY_MODEL` \| `BY_REGION` \| `BY_BATCH_SIZE` |
| 3 | **`controlVehicleWindow` 异步指令执行-确认语义缺失**：将"指令下发成功"与"车窗操作完成"混同，前端无法区分不同状态 | 一般 | (1) 在 §3.3 `controlVehicleWindow` 方法说明中明确返回值语义——`Ok(Unit)` 表示"指令已成功下发至 IoTDA"，不等同于车窗操作完成，前端应通过 `queryWindowStatus` 确认实际状态；(2) 在 S3 方法契约表中新增 `queryWindowStatus` 查询方法；(3) 在 §5.2 DriverStatusSnapshot 中新增 `windowStatus: Optional<List<WindowStatusEntry>>` 字段；(4) 新增车窗控制超时（30s 内未收到 IoTDA 执行确认 Ack）后标记 `TIMEOUT` 的异常处理策略；(5) 在 §4.3 新增 `QueryWindowStatusRequest`、`QueryWindowStatusResponse`、`WindowStatusEntry`、`WindowPosition`（enum）、`WindowState`（enum）、`WindowOperationResult`（enum）等 6 个 DTO/枚举类型；(6) 在 §6.1 AppError 新增 `WindowControlTimeout(vehicleId, operation)` 变体（C 类系统故障）；(7) 在 §6.2 错误策略表和 §7.4 事务边界表新增对应行 |
| 4 | **运营管理界面 DTO 缺少人类可读标识字段**：OfflineVehicleInfo、HighRiskDriverSummary、RescueRecordSummary 仅含 VehicleId/DriverId，不含车牌号和驾驶员姓名 | 一般 | 在 §4.4 OfflineVehicleInfo 中新增 `licensePlate: String` 和 `driverName: String`；在 §4.4 HighRiskDriverSummary 中新增 `driverName: String`；在 §4.5 RescueRecordSummary 中新增 `driverId: DriverId`、`driverName: String`、`licensePlate: String` 三个字段 |
| 5 | **CQRS 读模型访问机制在应用层未定义**：`queryAlertHistory` 和 `queryVehicleTrajectory` 标注"数据来源为 CQRS 读模型投影"但未定义访问端口/仓储/查询服务 | 轻微 | 在 §3.1 `queryAlertHistory` 方法说明中补充委托目标：「委托 AlertProjectionRepository 查询 CQRS 读模型投影」；在 §3.4 `queryVehicleTrajectory` 方法说明中补充委托目标：「委托 TrajectoryQueryService 查询 CQRS 读模型投影」 |
| 6 | **`QueryTrajectoryRequest` 双参数交叉校验缺失**：vehicleId 和 driverId 均为可选但约定"至少提供一个"，未覆盖二者同时提供且不匹配的场景 | 轻微 | 在 §4.4 QueryTrajectoryRequest 定义后新增参数交叉校验说明——若 vehicleId 和 driverId 同时提供但不匹配，返回空轨迹序列并标注 `dataConsistency = INCONSISTENT`，不返回 `AppError`（优先返回空结果而非中断查询流程）。在 §6.1 AppError 新增 `InvalidParameterCombination(detail)` 变体（A 类输入无效）作为其他参数组合校验的通用错误类型 |
| 7 | **离线消息队列 7 天告警保留策略未衔接隐私边界要求**：缺少加密存储、过期清除、家属权限过滤说明 | 轻微 | 在 §3.3 WebSocket 生命周期表"离线消息补推"行中补充三项隐私约束：(1) 离线消息队列中的告警数据加密存储（AES-256-GCM），到期（7 天）自动清除；(2) 补推时按当前有效授权范围过滤——仅推送家属仍持有监护权限的驾驶员告警；(3) 已撤销监护权限的家属账户不补推其在撤销前产生但未投递的告警 |
| 8 | **OTAManagementService 接口中 S6 依赖领域事件订阅声明不完整**：S6 协作关系中仅列出 3 个 OTA 状态事件，OTAUpgradeStartedEvent 未显式声明；S6 与 S2 的 OTA 事件消费分工未形成统一交叉引用 | 轻微 | (1) 在 §3.6 S6 协作关系事件订阅列表中补充 `OTAUpgradeStartedEvent`，并明确 S6 订阅全部四个 OTA 核心事件用于升级进度追踪、通知推送和历史记录更新；(2) 将 §7.2 链路 D 标题和正文重写为「OTA 升级期间干预抑制与事件消费分工」，以双订阅者模式分别描述 S6（进度追踪）和 S2（CAN 干预恢复）的 OTA 事件订阅分工，形成 S6 ↔ S2 的统一交叉引用 |
