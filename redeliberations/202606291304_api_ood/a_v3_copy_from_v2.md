# 车载安全监测系统 API/接口层 OOD 设计方案（a_v3 / v3）
...
> **应用层方法预签名（v3 新增端点映射）**：以下新增端点对应应用层 `IRemoteGuardianshipService` 的待新增方法，完整方法签名见 `docs/ood_application.md`：
...
> **应用层方法预签名（v3 新增端点映射）**：以下新增端点对应应用层 `IOTAManagementService` 的待新增方法，完整方法签名见 `docs/ood_application.md`：
