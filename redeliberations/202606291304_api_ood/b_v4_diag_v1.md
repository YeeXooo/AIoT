# API/接口层 OOD 设计质量审查报告（b_v4_diag_v1）
...
### 问题 1（严重·事实错误）：S3→S5 手动救援流转说明引用了不存在的应用层方法

- **所在位置**：§1.3 TriggerManualRescueResponse 字段说明（:208）及"S3→S5 手动救援流转说明"块（:211）
- **问题描述**：产出称"S3 RemoteGuardianshipService 调用 `S5 IEmergencyRescueService.createRescueReport()` 创建救援报告"。经核实，应用层 OOD（`docs/ood_application.md` §3.5）中 `IEmergencyRescueService` 的接口方法契约为 `confirmSOSReport`、`issueRescueToken`、`verifyRescueToken`、`queryRescueHistory` 四个方法，**不存在 `createRescueReport` 方法**。该名称在领域层（DS-12 EmergencyRescueService）中亦未出现。下游开发者按此说明实现时会发现目标方法不存在。
- **严重程度**：严重 — 阻断了 S3→S5 手动救援链路的实现落地，属于可阻断下游开发的引用错误。
- **改进建议**：核查救援报告创建的实际调用路径（是通过领域层 DS-12 的某个方法还是应用层 S5 的其他方法），将流转说明中的方法名修正为实际存在的方法签名；如确需新增该应用层方法，应同时在 `docs/ood_application.md` 中定义。
...
### 问题 5（轻微·安全性不一致）：车队大屏 WebSocket 端点使用 ws:// 而非 wss://

- **所在位置**：§4.2 看板数据订阅模型表（:1633）
- **问题描述**：车队大屏 WebSocket 端点写作 `ws://api.example.com/ws/fleet?token=<JWT>`，而家属 APP WebSocket 端点为 `wss://`（:1289）。§5.5 加密策略表明确规定"WebSocket 连接 — TLS 1.2+ (WSS)"，车队大屏应同样使用加密连接以确保传输安全。
- **严重程度**：轻微 — 属于演示环境 URL，但会误导下游实现者使用非加密 WebSocket，与安全设计矛盾。
- **改进建议**：将 §4.2 车队大屏 WebSocket URL 从 `ws://` 统一为 `wss://`。
...
产出已完成四轮迭代打磨，六个应用服务的 REST API 契约、MQTT 主题路由表、WebSocket/SparkRTC 信令协议、ArkTS 前端对接模型和安全设计五个部分均已覆盖，整体质量可用于下游接口层实现。但存在 1 项事实错误（引用不存在的应用层方法）、2 项跨层 DTO 不一致和 1 项关键细节缺失，需在第 5 轮修正后可交付使用。
