# 车载安全监测系统 API/接口层 OOD 设计方案（a_v6 / v6）
...
- `404` — 指定的 `tripId` 不存在时返回空集合（`activeInterventions: []`），不视为错误
- `503` — 服务不可用
...
> 
> **⚠ 接口契约待补**：步骤 (2) 引用的 `IEmergencyRescueService.createRescueReport()` 方法目前仅在本文档及应用层 OOD (`docs/ood_application.md` §4.3 TriggerManualRescueResponse) 中以 DTO 引用方式隐含存在，尚未在 `docs/ood_application.md` §3.5 `IEmergencyRescueService` 接口方法表中形式化定义（含方法签名、输入/输出 DTO、事务属性、异常处理）。下游 S3 实现者须待该接口形式化落地后方可完整实现编排逻辑。
> 
...
> **安全约束**：`POST /api/v1/sparkrtc/token` 端点按 §5.1 角色映射仅 FAMILY 角色可调用。后端须校验调用方 JWT 中的 `AccountRole` 与请求中 `role` 字段的一致性——FAMILY 角色调用方仅可请求 `role=subscriber`，不得通过传入 `publisher` 获取不应有的车机端权限。车机端的 `publisher` Token 通过 MQTT `cmd/media/join/down` 下发，不经此 REST 端点。
...
> **内部编排接口说明**：S5 的 `createRescueReport` 方法供 S3 RemoteGuardianshipService 在处理手动救援触发时内部编排调用（见 §1.3 S3→S5 手动救援流转说明），不对外暴露为独立的 REST 端点。该方法的方法签名和输入/输出 DTO 定义待 `docs/ood_application.md` §3.5 接口方法表补全后方可完整交叉引用。
...
| **救援触发确认** | `rescue_triggered` | `{ "rescueRequestId": "...", "rescueReportId": "...", "status": "..." }` | 手动救援触发确认（包含 S3 请求 ID 和 S5 救援报告 ID） |
...
| 项目 | 说明 |
|------|------|
| **豁免触发条件** | S1 RiskMonitoringService 判定 `COLLISION_DISABILITY`（碰撞失能）或 `LIFE_DETECTION_PROLONGED`（活体检测持续异常 ≥60s）→ 触发 `emergency_activation` 事件 |
...
interface IssueSparkRTCTokenReq {
  roomId: string
  userId: string
  role: SparkRTCRole                       // 'subscriber' | 'publisher'
}
