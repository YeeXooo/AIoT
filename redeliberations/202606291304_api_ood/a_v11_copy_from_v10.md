# 车载安全监测系统 API/接口层 OOD 设计方案（a_v11 / v11）
...
- `SMS_CODE` 模式：`credential` 为手机号，`secret` 为短信验证码
...
> **认证链路说明**：本端点是 JWT 认证链路的起点——客户端调用 `/api/v1/auth/login` 获取 `accessToken` 后，以 `Authorization: Bearer <accessToken>` 访问 §1.1–§1.6 的所有受保护端点。JWT 的签发（§5.1 步骤 1–2）、校验（步骤 3）、刷新（步骤 5）均依赖此端点产出的 Token。401 统一由 API 网关处理（见 §一 开篇约定），本端点自身无需认证头（`认证` 列标注 `—`）。

**错误响应**：
- `400` — 请求参数无效或 `authMethod` 不支持
- `401` — 认证失败（密码错误、短信验证码错误）
- `423` — 账户已锁定（连续 5 次密码错误后锁定 15 分钟）
- `429` — 登录尝试次数超限
