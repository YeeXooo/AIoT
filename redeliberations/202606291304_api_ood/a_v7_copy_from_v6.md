# 车载安全监测系统 API/接口层 OOD 设计方案（a_v8 / v8）
...
| 项目 | 说明 |
|------|------|
| **豁免触发条件** | S1 RiskMonitoringService 判定 `COLLISION_DISABILITY`（碰撞失能）或 `LIFE_DETECTION` 类型告警持续异常 ≥60s → 触发 `emergency_activation` 事件 |
