...
| `controlVehicleWindow` | `ControlVehicleWindowRequest` | `Result<Unit, AppError>` | — | 家属远程操控车窗开闭。安全门控：角色校验 → 二次身份验证 → 权限判定（委托 DS-08.compareAgainstScope 校核 windowOperation 是否在授权范围内）。授权通过后委托领域层执行车窗控制，通过 IoTDA 下发 CAN 指令至车辆 |
