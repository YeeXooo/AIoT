...
- `windowStatuses: Array<WindowStatusEntry>` — 各车窗状态列表

**WindowStatusEntry**
- `windowPosition: WindowPosition` — 车窗位置枚举
- `state: WindowState` — 车窗状态
- `lastOperation: Option<WindowControlOperation>` — 最近一次操作
- `lastOperationResult: Option<WindowOperationResult>` — 最近操作结果
...
- `offlineVehicles: Array<OfflineVehicleInfo>` — 脱线车辆列表
...
| `windowStatus` | `Option<Array<WindowStatusEntry>>` | 车窗状态列表（定义见 §4.3），用于家属查询车窗实时位置与控制操作结果 |
...
enum AppError {
    ...
    | InvalidParameterCombination(detail: String)
    ...
    | WindowControlTimeout(vehicleId: VehicleId, operation: WindowControlOperation)
}
...
| 2 | **`BatchStrategy` 类型完全未定义**：§4.6 UpgradeOptions 引用 `batchStrategy: Option<BatchStrategy>`，但全文未给出该类型定义 | 一般 | 在 §4.6 UpgradeOptions 之后新增 `BatchStrategy` 枚举类型定义：`BY_MODEL` \| `BY_REGION` \| `BY_BATCH_SIZE` |
| 3 | **`controlVehicleWindow` 异步指令执行-确认语义缺失**：将"指令下发成功"与"车窗操作完成"混同，前端无法区分不同状态 | 一般 | (1) 在 §3.3 `controlVehicleWindow` 方法说明中明确返回值语义——`Ok(Unit)` 表示"指令已成功下发至 IoTDA"，不等同于车窗操作完成，前端应通过 `queryWindowStatus` 确认实际状态；(2) 在 S3 方法契约表中新增 `queryWindowStatus` 查询方法；(3) 在 §5.2 DriverStatusSnapshot 中新增 `windowStatus: Option<Array<WindowStatusEntry>>` 字段；(4) 新增车窗控制超时（30s 内未收到 IoTDA 执行确认 Ack）后标记 `TIMEOUT` 的异常处理策略；(5) 在 §4.3 新增 `QueryWindowStatusRequest`、`QueryWindowStatusResponse`、`WindowStatusEntry`、`WindowPosition`（enum）、`WindowState`（enum）、`WindowOperationResult`（enum）等 6 个 DTO/枚举类型；(6) 在 §6.1 AppError 新增 `WindowControlTimeout(vehicleId, operation)` 变体（C 类系统故障）；(7) 在 §6.2 错误策略表和 §7.4 事务边界表新增对应行 |
