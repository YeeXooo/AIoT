...
**角色与职责**：EdgeSessionContext 是边缘侧 Spring Boot 应用中的**会话作用域 Bean**，作为单次行程（点火→熄火）期间所有边缘侧基础设施组件的生命周期容器和依赖注入枢纽。其核心职责是：(a) 管理 VehicleStateBuffer 和 PhysiologicalDataBuffer 的实例生命周期；(b) 持有当前行程的临时状态引用（如 `tripId`、`vehicleId`、`driverId`）；(c) 在行程结束时统一触发资源清理。

**Spring 集成方式**：

- **Bean 作用域**：定义为 Spring `@Scope(value = "custom", scopeName = "trip")` 自定义作用域，或简化为 `@Scope("singleton")` 的单一实例（边缘侧同一时刻仅一个活跃行程，熄火后销毁重建）。推荐后者——边缘侧进程级单例 + 手动生命周期管理，避免引入自定义 Scope 的复杂性。
- **创建触发**：由 `VehicleIgnitionOnEvent` 驱动——`infra.edge` 模块的 `SessionLifecycleManager`（`@Component`）订阅该事件，调用工厂方法 `EdgeSessionContext.create(tripId, vehicleId, driverId)` 实例化 EdgeSessionContext 并注入 VehicleStateBuffer 和 PhysiologicalDataBuffer 两个 ring buffer 实例。
...
- **轮转流程**：
  ...
  5. 90 天后（新密钥满一个周期），若旧版本关联的数据均已过期或被重新加密，DEW 自动删除旧版本密钥
