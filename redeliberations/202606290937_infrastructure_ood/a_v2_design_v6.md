...
边缘侧通过 MQTT 上行 Topic（`iot/{vehicleId}/alert/event`、`iot/{vehicleId}/sensor/data` 等）将告警事件、行程状态变更、轨迹数据上报至 IoTDA。IoTDA 规则引擎将上行消息转发至云端应用服务后，由 `infra.cloud` 模块中的 **IoTDA 上行消息消费适配器**（`IotdaUplinkConsumer`，`@Component`）负责：

- 消费 IoTDA 转发至应用服务的上行消息（通过 IoTDA REST API 订阅或规则引擎 HTTP 转发）
