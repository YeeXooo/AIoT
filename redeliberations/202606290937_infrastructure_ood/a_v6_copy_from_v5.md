# 车载安全监测系统 基础设施/适配器层 OOD 设计方案（v11）
...
| 约束项 | 值 | 说明 |
|--------|------|------|
| 单个会话最大持续时长 | 30 分钟 | 超过此时长后云端 `MediaSessionPort` 适配器主动调用 `terminateSession` 终止会话，防止异常长连接占用 SparkRTC 房间资源。终止前 1 分钟通过 WebSocket 推送"会话即将超时"通知至双方，家属端可主动续期（续期后重置计时器，最多续期 3 次） |
