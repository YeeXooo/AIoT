...
本任务（task_v2）动作为 RETRY，范围**仅限补齐 Maven 依赖**（需求 0.2），对象文件为 `code/server/pom.xml`。
...
**本轮设计无变更。** R1 验证失败原因为验证步骤误用仓颉构建工具 `cjpm test`（本项目为 Maven/Java 工程），与设计方案无关。设计规格维持 v1（含 r1 修订）不变。
...
- **验证说明**：使用 `mvn -f code/server/pom.xml test` 运行 JUnit 测试（**非** `cjpm test`，本项目为 Maven/Java 工程，不存在 cjpm.toml）。
...
| 审查意见 | 修改措施 |
|---------|---------|
| R1 验证失败：`Error: the file './cjpm.toml' does not exist` / `cjpm test failed`。原因为验证步骤误用了仓颉构建工具，本项目是 Maven/Java 17 工程 | 设计无变更。在本轮§行为契约中补充验证说明：应使用 `mvn -f code/server/pom.xml test` 而非 `cjpm test`。pom.xml 代码与 PomXmlTests.java 测试无需修改，`mvn dependency:resolve` 已 BUILD SUCCESS |
