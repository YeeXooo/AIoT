---
name: lint-governance-harness
description: "采用「检查-计划-执行」审议循环，对代码库进行渐进式lint治理。"
---

## 框架概述

三agent审议循环：checker运行lint工具诊断问题，planner形成增量治理计划，executor逐批修复。每轮聚焦一批问题，由checker独立验证修复效果，形成「诊断→计划→修复→验证」闭环。主Agent仅编排路由，禁止读取子Agent产出文件。

本技能不绑定任何具体 lint 工具。lint 命令通过 `lint_cmd` 参数注入；编译验证命令通过 `build_cmd` 参数注入；lint 处理指导原则通过 `guidelines_file` 参数传入（调用方提供，内容格式不限）。

子agent指令文档（启动时让agent自行阅读，不要将内容复制到prompt中）：

- [checker](./checker.md)：lint运行、结果解析与报告规范
...
- **`lint_cmd`**（必填）— lint检查命令，如 `eslint src/`、`flake8 src/`、`golangci-lint run ./...`、`cjlint -f src/` 等。checker 直接执行此命令
- **`build_cmd`**（可选）— 编译验证命令，executor 修复后用于验证编译通过，如 `mvn -q compile`、`cargo build`、`go build ./...` 等。若未提供，executor 跳过编译验证并在报告中标注
- **`guidelines_file`**（必填）— lint处理指导原则文件的绝对路径，由调用方提供。内容为各规则的处理策略和修复方式，供 planner 和 executor 参考
...
位于工作区 `governances/` 目录下，格式 `governances/{YYYYMMDDHHMM}_{简要描述}/`。
...
lint指导原则文件：{guidelines_file}
项目根目录：{project_root}
编译验证命令：{build_cmd，若未提供则注明"未提供，跳过编译验证"}
当前轮次：v{N}
```
...
- 通过检查：`Lint治理完成，代码库已通过lint检查。治理跟踪：{workdir}/governance.md，共经历 {N} 轮，分支：{branch}`
