...
本技能不绑定任何具体技术栈。设计中涉及的技术选型、库能力、语言特性等，由子agent依据需求中指定的目标技术栈自行查阅确认。
...
- **`requirement`**（必填）— 完整需求描述，初始化时写入 `{workdir}/requirement.md`。需求中应明确或包含目标技术栈，供子agent确认技术选型

### workdir 命名规范

运行 `date +%Y%m%d%H%M` 得到 `{YYYYMMDDHHMM}`。

workdir 应位于工作区的 `designs-tech/` 目录下，命名格式为 `designs-tech/{YYYYMMDDHHMM}_{简要描述}/`，例如 `designs-tech/202604041600_gh-pr-technical-design/`。
