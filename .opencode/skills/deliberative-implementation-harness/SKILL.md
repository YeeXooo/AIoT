---
name: deliberative-implementation-harness
...
本技能不绑定任何具体技术栈。编译与测试命令通过调用参数 `build_cmd` / `test_cmd` 注入；项目中源码与测试的组织方式、命名约定由子agent在运行时探索项目根目录后自行遵循。
...
| 参数 | 必填 | 说明 |
|------|------|------|
| `workdir` | 是 | 工作目录绝对路径，命名：`implements/{YYYYMMDDHHMM}_{简要描述}/`，`{YYYYMMDDHHMM}` 由 `date +%Y%m%d%H%M` 生成 |
| `requirement` | 是 | 整个任务的完整描述，初始化时写入 `{workdir}/requirement.md` |
| `project_root` | 是 | 项目根目录绝对路径 |
| `build_cmd` | 否 | 编译命令，用于 coder/executor 验证编译通过。若未提供，coder 自行按项目类型判断（如 `mvn -q compile`、`./gradlew compileJava`、`cargo build`、`go build ./...` 等），或跳过编译验证并在报告中标注 |
| `test_cmd` | 是 | 测试运行命令，runner 据此执行测试，如 `mvn test`、`./gradlew test`、`cargo test`、`go test ./...`、`pytest` 等。应在项目根目录或其约定子目录下执行 |
...
| `{workdir}/code_review_v{N}_r{CR}.md` | Code Reviewer |
...
workdir 应位于工作区的 `implements/` 目录下，命名格式为 `implements/{YYYYMMDDHHMM}_{简要描述}/`。
...
项目根目录：{project_root}
编译命令：{build_cmd，若未提供则注明"未提供，请按项目类型自行判断或跳过"}
